...
class UViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
                 ...
                 use_checkpoint=False, conv=True, skip=True, tokens=0, low_freqs=0, DCT_coes=0):
        super().__init__()
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        self.num_classes = num_classes
        self.tokens = tokens
        # self.DCT_coes = low_freqs * 6
        self.DCT_coes = DCT_coes

        self.proj = nn.Linear(self.DCT_coes, embed_dim, bias=True)
        ...
        self.decoder_pred = nn.Linear(embed_dim, self.DCT_coes, bias=True)
